...
type OrgsApiListConstraintsRequest struct {
	ctx        _context.Context
	ApiService *OrgsService
	orgHandle  string
	where      *string
...
func (r OrgsApiListConstraintsRequest) Where(where string) OrgsApiListConstraintsRequest {
	r.where = &where
	return r
}

// The max number of items to fetch per page of data, subject to a min and max of 1 and 100 respectively. If not specified will default to 25.
func (r OrgsApiListConstraintsRequest) Limit(limit int32) OrgsApiListConstraintsRequest {
	r.limit = &limit
	return r
}

// When list results are truncated, next_token will be returned, which is a cursor to fetch the next page of data. Pass next_token to the subsequent list request to fetch the next page of data.
func (r OrgsApiListConstraintsRequest) NextToken(nextToken string) OrgsApiListConstraintsRequest {
	r.nextToken = &nextToken
	return r
}

func (r OrgsApiListConstraintsRequest) Execute() (ListConstraintsResponse, *_nethttp.Response, error) {
	return r.ApiService.ListConstraintsExecute(r)
}

/*
ListConstraints List org constraints

List all applicable constraints for an org.

	@param ctx _context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param orgHandle Specify the handle of the org whose constraints you want to retrieve.
	@return OrgsApiListConstraintsRequest
*/
func (a *OrgsService) ListConstraints(ctx _context.Context, orgHandle string) OrgsApiListConstraintsRequest {
	return OrgsApiListConstraintsRequest{
		ApiService: a,
		ctx:        ctx,
		orgHandle:  orgHandle,
...
func (a *OrgsService) ListConstraintsExecute(r OrgsApiListConstraintsRequest) (ListConstraintsResponse, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod  = _nethttp.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "OrgsService.ListConstraints")
	if err != nil {
		return localVarReturnValue, nil, GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/org/{org_handle}/constraint"
	localVarPath = strings.Replace(localVarPath, "{"+"org_handle"+"}", _neturl.PathEscape(parameterToString(r.orgHandle, "")), -1)
